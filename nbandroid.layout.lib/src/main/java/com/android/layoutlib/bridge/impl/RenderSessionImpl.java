 ...
 * <p/>
 * A session is a stateful representation of a layout file. It is initialized
 * with data coming through the {@link Bridge} API to inflate the layout.
 * Further actions and rendering can then be done on the layout.
 */
public class RenderSessionImpl extends RenderAction<SessionParams> {
    ...
    /**
     * Creates a layout scene with all the information coming from the layout
     * bridge API.
     * <p>
     * This <b>must</b> be followed by a call to
     * {@link RenderSessionImpl#init(long)}, which act as a call to
     * {@link RenderSessionImpl#acquire(long)}
     *
    ...
    /**
     * Initializes and acquires the scene, creating various Android objects such
     * as context, inflater, and parser.
     *
    ...
    /**
     * Measures the the current layout if needed (see
     * {@link #invalidateRenderingSize}).
     */
     ...
     *
     * @throws IllegalStateException if the current context is different than
     * the one owned by the scene, or if {@link #init(long)} was not called.
     */
    ...
    /**
     * Sets the time for which the next frame will be selected. The time is the
     * elapsed time from the current system nanos time. You
     */
    ...
    /**
     * Renders the given view hierarchy to the passed canvas and returns the
     * result of the render operation.
     *
     * @param canvas an optional canvas to render the views to. If null, only
     * the measure and layout steps will be executed.
     */
     ...
     * {@link #acquire(long)} must have been called before this.
     *
     * @param freshRender whether the render is a new one and should erase the
     * existing bitmap (in the case where bitmaps are reused). This is typically
     * needed when not playing animations.)
     *
     * @throws IllegalStateException if the current context is different than
     * the one owned by the scene, or if {@link #acquire(long)} was not called.
     *
    ...
    /**
     * Executes {@link View#measure(int, int)} on a given view with the given
     * parameters (used to create measure specs with
     * {@link MeasureSpec#makeMeasureSpec(int, int)}.
     *
     * if <var>measuredView</var> is non null, the method returns a {@link Pair}
     * of (width, height) for the view (using {@link View#getMeasuredWidth()}
     * and {@link View#getMeasuredHeight()}).
     *
     * @param viewToMeasure the view on which to execute measure().
     * @param measuredView if non null, the view to query for its measured
     * width/height.
     ...
     * @return the measured width/height if measuredView is non-null, null
     * otherwise.
     */
     ...
     * <p/>
     * At the moment this only supports TabHost: If {@link TabHost} is detected,
     * look for the {@link TabWidget}, and the corresponding {@link FrameLayout}
     * and make new tabs automatically based on the content of the
     * {@link FrameLayout}.
     *
    ...
    /**
     * If the root layout is a CoordinatorLayout with an AppBar: Set the title
     * of the AppBar to the title of the activity context.
     */
    ...
    /**
     * Set the scroll position on all the components with the "scrollX" and
     * "scrollY" attribute. If the component supports nested scrolling attempt
     * that first, then use the unconsumed scroll part to scroll the content in
     * the component.
     */
     ...
     * @param layoutlibCallback The project callback object to access the
     * project R class.
     * @throws PostInflateException if TabHost is missing the required ids for
     * TabHost
     */
    ...
    /**
     * Visits a {@link View} and its children and generate a {@link ViewInfo}
     * containing the bounds of all the views.
     *
     * @param view the root View
     * @param hOffset horizontal offset for the view bounds.
     * @param vOffset vertical offset for the view bounds.
     * @param setExtendedInfo whether to set the extended view info in the
     * {@link ViewInfo} object.
     * @param isContentFrame {@code true} if the {@code ViewInfo} to be created
     * is part of the content frame.
     *
     * @return {@code ViewInfo} containing the bounds of the view and it
     * children otherwise.
     */
    ...
    /**
     * Visits all the children of a given ViewGroup and generates a list of
     * {@link ViewInfo} containing the bounds of all the views. It also
     * initializes the {@link #mViewInfoList} with the children of the
     * {@code mContentRoot}.
     *
     ...
     * {@link ViewInfo} object.
     * @param isContentFrame {@code true} if the {@code ViewInfo} to be created
     * is part of the content frame. {@code false} if the {@code ViewInfo} to be
     * created is part of the system decor.
     */
    ...
    /**
     * Visits the children of {@link #mContentRoot} and generates
     * {@link ViewInfo} containing the bounds of all the views. It returns two
     * {@code ViewInfo} objects with the same children, one with the
     * {@code offset} and other without the {@code offset}. The offset is needed
     * to get the right bounds if the {@code ViewInfo} hierarchy is accessed
     * from {@code mViewInfoList}. When the hierarchy is accessed via
     * {@code mSystemViewInfoList}, the offset is not needed.
     *
     * @return an array of length two, with ViewInfo at index 0 is without
     * offset and ViewInfo at index 1 is with the offset.
     */
    @NonNull
    ...
    /**
     * Creates a {@link ViewInfo} for the view. The {@code ViewInfo}
     * corresponding to the children of the {@code view} are not created.
     * Consequently, the children of {@code ViewInfo} is not set.
     *
     * @param hOffset horizontal offset for the view bounds. Used only if view
     * is part of the content frame.
     * @param vOffset vertial an offset for the view bounds. Used only if view
     * is part of the content frame.
     */
