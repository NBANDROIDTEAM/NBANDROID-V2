...
import android.studio.imports.templates.Parameter.Constraint;
...
/**
 * Class which handles setting up the relationships between a bunch of
 * {@link Parameter}s and then resolves them.
 */
public final class ParameterValueResolver {
    ...
    /**
     * Resolve input parameters, returning a mapping of parameters to their
     * resolved values.
     *
     * @param parameters parameters to resolve
     * @param userValues parameter values supplied by the user.
     * @param additionalValues parameters that were not declared by the template
     * but are instead a part of our "runtime"
     * @param deduplicator a function that ensures uniqueness of the parameter
     * value
     */
    @NotNull
    public static Map<Parameter, Object> resolve(@NotNull Iterable<Parameter> parameters,
     ...
     *
     * These parameters may depend on other computable parameters. We do not
     * have that information (expressions are evaluated with FreeMarker), so we
     * keep reevaluating the parameter values until they stabilize.
     */
    @NotNull
