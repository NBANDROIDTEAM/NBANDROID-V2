...
public class AndroidProjectTemplatePanelVisualAndroidSettings extends JPanel implements ItemListener, DocumentListener {
    ...
    private void updateDistribution() {
        PlatformDecorator selectedItem = (PlatformDecorator) phonePlatforms.getSelectedItem();
    ...
    void read(WizardDescriptor settings) {
        ...
        SdkManagerPlatformPackagesRootNode platformPackagesRootNode = androidSdk.getPlatformPackages();
        Enumeration childrens = platformPackagesRootNode.children();
        List<PlatformDecorator> versionDecorators = new ArrayList<>();
        childrens.asIterator().forEachRemaining((child) -> {
            if (child instanceof AndroidVersionNode) {
                versionDecorators.add(new PlatformDecorator((AndroidVersionNode) child, androidSdk));
            }
        });

        phonePlatforms.setModel(new DefaultComboBoxModel(versionDecorators.toArray()));
        phonePlatforms.setRenderer(new ComboBoxRenderer());
        for (Iterator<PlatformDecorator> iterator = versionDecorators.iterator(); iterator.hasNext();) {
            PlatformDecorator next = iterator.next();
        ...
        wearPlatforms.setModel(new DefaultComboBoxModel(versionDecorators.toArray()));
        ...
        tvPlatforms.setModel(new DefaultComboBoxModel(versionDecorators.toArray()));
        ...
        if (settings.getProperty(PROP_PHONE_TABLET_PLATFORM) instanceof PlatformDecorator) {
            phonePlatforms.setSelectedItem((PlatformDecorator) settings.getProperty(PROP_PHONE_TABLET_PLATFORM));
        }
        if (settings.getProperty(PROP_WEAR_PLATFORM) instanceof PlatformDecorator) {
            wearPlatforms.setSelectedItem((PlatformDecorator) settings.getProperty(PROP_WEAR_PLATFORM));
        }
        if (settings.getProperty(PROP_TV_PLATFORM) instanceof PlatformDecorator) {
            tvPlatforms.setSelectedItem((PlatformDecorator) settings.getProperty(PROP_TV_PLATFORM));
        }
    ...
    class ComboBoxRenderer extends BasicComboBoxRenderer {
        ...
        public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
            ...
            PlatformDecorator info = (PlatformDecorator) value;
