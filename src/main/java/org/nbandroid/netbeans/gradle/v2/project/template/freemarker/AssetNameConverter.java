...
/**
 * Allows a one to one mapping suggestion between different types of Android
 * asset names, like for example mapping the name of an Activity to its layout.
 * e.g. an Activity with name "ActivityMain" may have a suggested layout name of
...
public class AssetNameConverter {
    ...
    /**
     * Prefix added to default layout names. Can be overridden via
     * {@link #overrideLayoutPrefix(String)}
     */
    private static final String DEFAULT_LAYOUT_NAME_PREFIX = "activity";

    /**
     * When stripping the Activity suffix, we match against "Activity" plus zero
     * or more digits. The base of the name will be captured in the first group
     * and the digits will be captured in the second group.
     */
    ...
    /**
     * Convert whatever current text type we're representing into the
     * {@link Type#CLASS_NAME} type, since that can act as a common base type we
     * can use to reliably covert into all other types.
     */
    @NotNull
        ...
        switch (myType) {
            ...
            case CLASS_NAME:
                ...
                for (String suffix : STRIP_CLASS_SUFFIXES) {
                    className = TemplateUtils.stripSuffix(className, suffix, true);
                }
                if (myLayoutPrefixOverride != null) {
                    String prefixAsSuffix = TemplateUtils.underlinesToCamelCase(myLayoutPrefixOverride);
                    className = TemplateUtils.stripSuffix(className, prefixAsSuffix, false);
                }
                break;
    ...
    /**
     * Override the default layout prefix. This should <i>not</i> include its
     * trailing underscore. This will only be used when converting from or to
     * the {@link Type#LAYOUT} type.
     *
    ...
    /**
     * Strip the "Activity" suffix from a class name, e.g. "EditorActivity" ->
     * "Editor". This does not strip recursively, so "EditorActivityActivity" ->
     * "EditorActivity"
     *
     * Because Studio suggests appending numbers onto new classes if they have a
     * duplicate name, e.g. "MainActivity", "MainActivity2", "MainActivity3", we
     * take that into account, for example we would convert "MainActivity3" into
     * "Main3"
     */
    private static String stripActivitySuffix(@NotNull String activityName) {
        String finalName = TemplateUtils.stripSuffix(activityName, ACTIVITY_NAME_SUFFIX, false);
