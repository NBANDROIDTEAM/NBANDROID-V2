 ...
 *
 * Matches to {@link SdkManager} but has to be initialized first. No platform is
 * returned until SDK location is set (usually when project is opened or from
 * UI).
 *
 * @author jesse, radim
 */
public final class DalvikPlatformManager {
    ...
    private void readPlatforms() {
        ...
        mgr = AndroidSdkHandler.getInstance(new File(sdkLocation));
        if (mgr == null) {
