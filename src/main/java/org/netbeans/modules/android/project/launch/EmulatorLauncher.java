...
import org.netbeans.modules.android.google.PropertyFetcher;
                ...
                synchronized (connectLock) {
                    while (true) {
                        for (IDevice device : connectedDevices) {
                            if (device.isOnline()) {
                                PropertyFetcher fetcher = new PropertyFetcher(device);
                                ProgressHandle handle = ProgressHandle.createHandle("Emulator starting");
                                handle.start();
                                while (true) {
                                    String property = fetcher.getProperty("sys.boot_completed").get();
                                    if (property != null) {
                                        break;
                                    }
                                }
