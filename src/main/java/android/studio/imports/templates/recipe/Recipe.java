...
import org.nbandroid.netbeans.gradle.v2.project.template.freemarker.TemplateUtils;
...
public class Recipe implements RecipeInstruction {
    ...
    /**
     * A "folder" attribute can be specified on a recipe tag to handle loading
     * of relative files from a recipe that is included inside another recipe
     * file. If not specified the value will be null.
     */
    ...
    /**
     * Handles parsing a recipe.xml file. A recipe file specifies a bunch of
     * file-related actions to take after a template is processed, such as
     * copying files over, merging them, or opening them in the main editor.
     * Once parsed, you should remember to {@link #execute(RecipeExecutor)} it.
     */
    ...
    /**
     * Appends a gradle update instruction. This should only be manually added
     * if not using {@code #parse}, such as when your recipe is a node in a
     * larger xml file rather than a recipe.xml file being passed in.
     */
